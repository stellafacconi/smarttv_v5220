...
const GROUP_COLORS = ['#34c759', '#ff375f', '#0a84ff']

interface Props {
  onBack: () => void
  onCreated: (name: string, members: { color: string }[]) => void
}

type Focus = 'single' | 'group'

export default function GroupCreate({ onBack, onCreated }: Props) {
  ...
  const createGroup = () => {
    if (!canSelect) return
    onCreated('Group', GROUP_COLORS.map(color => ({ color })))
  }
  ...
  useEffect(() => {
    const t = setTimeout(() => setCanSelect(true), 350)
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter' || e.key === ' ' || e.key === 'e' || e.key === 'E') {
        e.preventDefault()
        if (focus === 'group') createGroup()
        else if (canSelect) onBack()
      }
    }
    window.addEventListener('keydown', h)
    return () => window.removeEventListener('keydown', h)
  }, [focus, onBack, canSelect])
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          ...
          lineHeight: '22px',
          letterSpacing: 0.68,
          color: focused ? '#131111' : 'rgba(255,255,255,0.90)',
          whiteSpace: 'nowrap',
        }}>
          {label}
        </span>
